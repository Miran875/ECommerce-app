...
import com.imran.gshop.databinding.ActivityRegisterBinding
import com.imran.gshop.model.UserModel
...
class RegisterActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btnLoginoutln.setOnClickListener{
           openLogin()
        }
    ...
    private fun validateUser() {
        if (binding.userName.text!!.isEmpty()|| binding.userNumber.text!!.isEmpty()){
            Toast.makeText(this, "Please enter login credentials properly", Toast.LENGTH_SHORT).show()
        }else{
            storeData()
        }
    }
